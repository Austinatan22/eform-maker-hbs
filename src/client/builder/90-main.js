...
(function () {
  ...
  class Builder {
    constructor() {
      ...
      this._sortableInstance = null; // Track SortableJS instance
    }
    ...
    installUnloadGuard() {
      ...
      this._eventListeners.push({ element: window, event: 'beforeunload', handler: unloadHandler });
    }
    ...
    bindEvents() {
      ...
      if (this.$.quickAddButtons) {
        this.$.quickAddButtons.addEventListener('click', quickAddHandler);
        this._eventListeners.push({ element: this.$.quickAddButtons, event: 'click', handler: quickAddHandler });
      }
      ...
      if (this.$.presetButtons) {
        this.$.presetButtons.addEventListener('click', presetHandler);
        this._eventListeners.push({ element: this.$.presetButtons, event: 'click', handler: presetHandler });
      }
      ...
      if (this.$.preview) {
        this.$.preview.addEventListener('click', previewClickHandler);
        this._eventListeners.push({ element: this.$.preview, event: 'click', handler: previewClickHandler });
      }
    ...
    initSortable() {
      ...
      try {
        ...
        this._sortableInstance = new window.Sortable(host, {
          animation: 150,
          draggable: '[data-fid]',
